...
public class FileProcessorServiceTest {
    ...
    @Test
    public void processInputTransactionsFile() throws Exception{
        ...
        Mockito.doNothing().when(kafkaProducerService).publishNotification(Mockito.anyString());

        fileProcessorService.processInputTransactionsFile();

        verify(jobLauncher,times(1)).run(Mockito.any(), Mockito.any());
        verify(kafkaProducerService, times(1)).publishNotification(Mockito.anyString());
    }
}
